...
def resolve_task(plan: Plan, ref: str) -> Task:
    ...
    # Ссылка вида «#4 «Разработка методик»» может противоречить себе: номер —
    # это позиция, и после переупорядочивания он указывает на другую задачу.
    # Название устойчиво, поэтому при конфликте верим ему. Разбираем до снятия
    # кавычек, иначе закрывающая кавычка исчезнет и шаблон не сработает.
    combined = re.fullmatch(r"#?\s*\d{1,4}\s*[«\"'](?P<name>.+)[»\"']", needle)
    if combined:
        needle = combined.group("name")
