...
| Раскрыть агента на весь экран | Иконка в правом верхнем углу шапки чата |
| Позвать агента на телефоне | Кнопка «Агент» над планом — чат выезжает поверх, закрывается крестиком или Escape |
| На телефоне: список вместо полос | Переключатель «Диаграмма / Список» над планом |
...
| Понять, где вы в проекте | Рамка на спайне под диаграммой — видимая часть плана, движется вместе с прокруткой |
...
на спайне). Полоска под диаграммой — мини-карта всего проекта: диаграмма показывает окно, а
спайн — целое, где плотность работ и где идёт критический путь. Зелёная рамка на спайне —
видимая часть плана: она едет вместе с прокруткой диаграммы, так что видно, где вы в проекте.
...
**На телефоне два вида плана.** По умолчанию — диаграмма, и колонка названий там скрыта совсем:
таймлайн получает все 400&nbsp;px вместо 204&nbsp;px, а название задачи рисуется подписью у её
полоски. Переключателем рядом открывается список карточек: номер, цветная метка статуса, полное
название, срок, длительность, исполнитель, статус, запас или пометка «критический путь» и полоска
прогресса. Тап по карточке открывает ту же модалку правки, что на большом экране. Раньше экран
делился пополам — 196&nbsp;px названий и 204&nbsp;px полос — и ни одной половины не хватало.

**Агент раскрывается на весь экран** иконкой в правом верхнем углу своей шапки: чат здесь основной
инструмент правок, и иногда нужен целиком, а не полосой сбоку. Содержимое при этом ограничено по
ширине, чтобы строки не растягивались на весь монитор.

**На телефоне у агента нет полосы внизу — он выезжает поверх плана** по кнопке «Агент» над планом
и закрывается крестиком, Escape или системным «назад». Свёрнутая полоса отнимала у диаграммы 70&nbsp;px
и ничего не показывала; теперь план и чат по очереди занимают весь экран. Пока запрос в работе,
кнопка «Агент» мигает — с закрытой панелью это единственный признак, что агент ещё думает.

**На телефоне другая механика управления, а не уменьшенная настольная.** На экране в 230 пикселей
таймлайна окно просмотра всё равно упиралось в минимальную ширину колонки, и ползунок перестал
что-либо менять — поэтому там вместо шкалы три фиксированных шага: дни, недели, месяцы. В списке
остаётся только колонка с названием (настройки колонок не теряются и вернутся на большом экране),
подписи кнопок сворачиваются в иконки, у спайна прячется легенда. Панели управления над планом
собраны в два коротких ряда — виды и кнопка агента, затем поиск, фильтры и масштаб выпадающим
списком: тремя табами масштаб не влезал и выдавливал фильтры за край экрана.

Жесты внутри диаграммы ведём сами, обе оси. Сдвиг в сторону прокручивает по времени, сдвиг
вверх-вниз — строки диаграммы (ось определяется по первым шести пикселям движения). Вертикаль
раньше не работала вовсе: у `.chart__frame` стояло `touch-action: pan-y`, браузер забирал
вертикальный жест себе, не находил, что прокручивать, и обрывал жест целиком. Теперь
`touch-action: none`, а прокрутку делаем через собственный обработчик колеса библиотеки — это
единственный путь, которым она соглашается сдвинуться на произвольное число пикселей: прямая
запись `scrollTop`/`scrollLeft` глотается её флагом `ignoreScrollEvent`, а проп `viewDate` умеет
только целые колонки. Из-за этих колонок горизонтальная прокрутка на телефоне и шла рывками — по
64&nbsp;px за раз на неделях; теперь жест переводится в колесо и диаграмма идёт пиксель в пиксель.
На большом экране горизонтальный жест по-прежнему двигает окно шкалы, чтобы шкала и диаграмма не
разъезжались.

Позицию прокрутки для бегунка на спайне читаем не из окна шкалы, а у самой библиотеки — событие
`scroll` её горизонтального скроллбара сводит все источники (шкала, колесо, жесты) в одно место.
Заодно жест на телефоне теперь упирается в финиш проекта: таймлайн библиотеки тянется на год
дальше, и без ограничения свайп уезжал в пустоту — на большом экране такое ограничение было в
пересчёте окна шкалы, а на телефоне прокрутка идёт мимо него.

Нажатие с удержанием (420&nbsp;мс) на строке списка поднимает задачу для переноса — это работает
там, где виден список задач, то есть на большом экране; на телефоне в режиме диаграммы колонки
списка нет, и порядок меняется либо на большом экране, либо через агента («поставь #7 после #3»).
Обработчики жестов навешиваются один раз, а свежие колбэки читаются через ref: пока они висели в
эффекте с зависимостями, любая перерисовка снимала слушатели вместе с таймером удержания, и жест
не срабатывал.
...
**Шапка — только факты о плане.** Из неё убраны вводные надписи «План проекта» и «сессия …»:
первая повторяла заголовок, вторая нужна раз в жизни (идентификатор остался в подсказке к
заголовку). Спайн уехал из шапки под диаграмму, где он и читается как мини-карта: сверху окно,
снизу целое. Шапка стала 80&nbsp;px вместо 186 на большом экране и 135 вместо 256 на телефоне —
эта высота ушла диаграмме.
...
намеренно нестандартный элемент, спайн проекта под диаграммой, сохранён и перекрашен токенами.
