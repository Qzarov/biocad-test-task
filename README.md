...
| Прокрутить таймлайн | Ползунок под диаграммой (или колесо мыши с Shift) |
| Изменить масштаб | Ползунок «Масштаб» — плотность внутри выбранного шага (день/неделя/месяц) |
...
| Вернуться к переписке | Переписка хранится на сервере и подгружается после перезагрузки страницы; «Очистить» стирает её |
...
задача, которую только что изменил агент. Красный в системе означает ошибку, поэтому на
диаграмме он не используется. Полоска в шапке («спайн проекта») показывает всю длительность
...
┌───────────────▼──────────────────────────────────────────────────┐
│  MCP-сервер (app/mcp_server): 12 инструментов над планом         │
...
`shift_task`, `move_task_to`, `unpin_task`, `reassign_tasks`, `reorder_task`, `set_project_start`.
...
и в `get_plan`, и `resolve_task` понимает «#5». При переупорядочивании номера меняются, поэтому
это ссылка «здесь и сейчас», а не идентификатор: в тексте сообщения рядом с номером
подставляется и название, так что старое сообщение остаётся понятным.
